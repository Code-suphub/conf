    ...
    for i,rho2 in enumerate(rho2_lst):
        try:
            with open(path_pre + str(0.001) + f"_rho2_{rho2}.csv", 'r') as f:
                data.append([eval(i) for i in f.read().split("---")[1:]])
            plt.plot([i for i in range(len(data[-1]))], [i[0] for i in data[-1]], label=f"rho2={rho2}", linestyle = style[i%len(style)])
        except Exception as e:
            pass
    plt.xlabel("iteration round")
    plt.ylabel("target function u_t")
    plt.title("[Coordinate Polling] target function VS iteration round")
    ...
    for i,rho2 in enumerate(rho2_lst):
        try:
            with open(path_pre + str(0.001) + f"_rho2_{rho2}.csv",
                      'r') as f:
                data.append([eval(i) for i in f.read().split("---")[1:]])
            plt.plot([i for i in range(len(data[-1]))], [i[1] for i in data[-1]], label=f"rho2={rho2}", linestyle = style[i%len(style)])
        except Exception as e:
            pass
    plt.xlabel("iteration round")
    plt.ylabel("batch-size")
    plt.title("[Coordinate Polling] all devices batch-size summation VS iteration round")
