...
if __name__ == '__main__':
    ...
    file_name = f"../save/output/conference/cmpResult/rho/local_cnt[1]_user30_rho1[{rho}]_rho2[{rho2}]_alpha[{alpha}]_origin2.csv"
    cnt = 0
    sumT=0
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        while True:
            ...
            #
            # ut_value, total_delay = algo.cal_old_ut()  # 归一化求解
            algo.batch_size_decision2(epoch = epoch, countt = cnt, log=False,log2=log2)
            ut_value, total_delay = algo.cal_ut()  # 归一化求解
        ...
        for idx, a in enumerate(fl_lst):
            if a == 1:
                print("FL：ind:", idx, '---------------- batch_size_lst: ', algo.batch_size_lst[idx], '------------',
                      len(sample_data[idx]), '-------------- SL number: ', sum(algo.sl_lst))
        ...
        slBatch = [algo.batch_size_lst[i] if algo.sl_lst[i] == 1 else 0 for i in range(algo.user_num)]
        ...
        try:
            ...
            res[epoch].append(slBatch)
            res[epoch].append(sum(algo.batch_size_lst))
        except:
            time.sleep(1000000)
        try:
            ...
            with open(file_name, 'w') as f:
                json.dump(str(res), f)
        sumT+=time.time()-t1
        # td+= str(epoch)+","+str(time.time()-t1) + ","+str(sumT) + ","+str(sum(algo.batch_size_lst))+ ","+str(sum(slBatch))+"\n"
